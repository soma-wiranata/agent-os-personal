...
3. Ask the user (using `AskUserQuestion` / `ask_question` or in text) to present the areas:
...
3. Ask the user (using `AskUserQuestion` / `ask_question` or in text) to present findings and let user select:
...
1. **Ask 1-2 clarifying questions** about the "why" behind the pattern. Use `AskUserQuestion`, `ask_question`, or ask directly in your text response.
...
3. Draft the content and ask the user (using `AskUserQuestion` / `ask_question` or in text) to confirm:
...
2. For each new file without an index entry, ask the user (using `AskUserQuestion` / `ask_question` or in text):
