...
**If any files exist**, ask the user (using `AskUserQuestion` / `ask_question` or in text):
...
After they respond, ask the user (using `AskUserQuestion` / `ask_question` or in text):
...
**If the tech-stack standard exists**, read it and ask the user (using `AskUserQuestion` / `ask_question` or in text):
...
**If no tech-stack standard exists** (or they chose option 2 above), ask the user (using `AskUserQuestion` / `ask_question` or in text):
